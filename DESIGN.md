...
A transaction has a timestamp, input list, output list, and `tx_id`. Its identifier is a non-empty SHA-256 digest string of those fields, and ledger inspection recalculates it so an edited stored transaction or malformed transaction ID is reported rather than crashing the inspection. A transaction ID must appear only once across confirmed and pending records, so a block reference always names one unambiguous transaction. Each output ID must also be unique across confirmed and pending records; otherwise one output could silently replace another in the UTXO map. When building a new transfer, outputs already referenced by a pending transfer are temporarily reserved, so an obvious double-spend is rejected immediately rather than waiting for mining. Blocks contain a height, timestamp, parent hash, and a list of transaction identifiers. This intentionally avoids a Merkle tree so the link between a block and its included transactions remains obvious.
